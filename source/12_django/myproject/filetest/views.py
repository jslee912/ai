...
def upload_file(request):
  if request.method == "POST":
    ...
    MAX_FILE_SIZE = 1024 * 1024 * 10 # 10MB  # 최대 파일 용량 10MB
    if upload_file and upload_file.size < MAX_FILE_SIZE:
      ...
      # 최대 파일 용량 10MB 이상 파일 첨부함
      messages.error(request, 
                     f"파일사이즈({int(upload_file.size/(1024*1024))})KB)가 10M를 초과할 수 없습니다.")
      messages.warning(request,"10M를 초과할 수 없습니다.")
    else:
      # 파일 첨부 안함
      messages.success(request, "파일을 첨부하지 않았습니다.")
      messages.info(request, "파일을 첨부하지 않았습니다.")
  return render(request, "filetest/fileupload.html")

import time
def predict(request):
  time.sleep(3)  # predict 시간이라 칩시다.
