...
-- 1.SELECT 문장 작성법
SELECT * FROM TAB; --현재 계정(SCOTT)이 가지고 있는 테이블 정보(실행:CTRL+ENTER)
...
DESC EMP;
    -- EMP 테이블의 구조
SELECT EMPNO, ENAME, SAL, JOB,HIREDATE FROM EMP;  -- EMP 테이블의 SELECT절에 지정된 열만 출력
...
--4.WHERE절(조건절)에 논리연산자 : AND OR NOT
    ...
    -- 날짜 표기법 셋팅 (현재:RR/MM/DD) RR은 현세기 기준으로 50보다 크면 1900년대 50보다 작으면 2000년대
...
SELECT EMPNO, ENAME, SAL "예전월급", SAL*1.1 "현재월급" FROM EMP;
    ...
    SELECT ENAME, SAL, SAL*12 ANNUALSAL --(3)화면에 출력
        FROM EMP                       -- (1)테이블을 가져온다
        WHERE SAL*12 > 10000           -- (2)조건에 만족하는 레코드만 메모리에 올린다
        ORDER BY ANNUALSAL;            -- (4)정렬해서 출력
    -- 산술연산의 결과는 NULL을 포함하면 결과도 NULL
    -- NVL(NULL일 수도 있는 필드명,대체값)을 이용 : 필드명과 대체값은 타입이 일치
    -- ex2. 직원의 이름, 월급, 상여, 연봉(SAL*12+COMM)
    SELECT ENAME, SAL, COMM, SAL*12+COMM 연봉 FROM EMP; -- 연봉이 NULL인 경우 발생됨
    SELECT ENAME, SAL, COMM, SAL*12+NVL(COMM,0) 연봉 FROM EMP;
    -- ex3. 모든 사원의 ENAME, MGR(상사의사번)을 출력-상사사번이 없으면 'CEO'
    SELECT ENAME, NVL(TO_CHAR(MGR),'CEO') FROM EMP;  -- MGR과'CEO'의 타입이 일치하지 않아서 
    DESC EMP;
...
-- 	연습문제 꼭 풀기
    ...
    SELECT '"' || ENAME ||'은  ' || JOB ||'업무이고  연봉은 '|| (SAL*12+NVL(COMM,0))||'이다' MESSAGE
    FROM EMP ;
...
-- (1)필드명 BETWEEN A AND B : 필드명이 A부터 B까지(A,B포함. A<=B)
    ...
    SELECT * FROM EMP WHERE SAL BETWEEN 3000 AND 1500; -- 반드시 작은값이 앞에(X)
...
-- ex1-1. ex1의 반대(부서코드가 10번도 아니고, 30도 아니고 40도 아닌사람)
SELECT * FROM EMP WHERE DEPTNO NOT IN (10,30,40);

-- (3)필드명 LIKE '패턴' : %(0글자이상), _(1글자)를 포함하는 패턴
    ...
    -- ex. 이름에 %가 들어간 사원
    SELECT * FROM EMP WHERE ENAME LIKE '%\%%' ESCAPE '\'; -- \이외의 문자도 가능하나 관례상 \를 사용
    DESC EMP;
        -- 이름에 %가 들어간 데이터 INSERT        
        INSERT INTO EMP VALUES (9998,'김%동',NULL,NULL,NULL,9000,9000,40);
    SELECT *  FROM EMP;
...
--출력하는 SELECT 문을 작성
    ...
    FROM EMP
    WHERE COMM>SAL*1.1;  -- WHERE NVL(COMM,0) >= SAL*1.1
