...
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.*
    ...
    -- ex.JOB이 'SALESMAN'이거나 'MANAGER'인 사원의 이름, 급여, 상여, 연봉, 부서명
        ...
        SELECT ENAME, SAL, COMM, ((SAL+NVL(COMM,0))*12) ANNSAL, D.DNAME
            FROM EMP E, DEPT D
            WHERE E.DEPTNO = D.DEPTNO AND JOB IN ('SALESMAN','MANAGER')
            ORDER BY ANNSAL DESC;  -- ORDER BY (SAL+NVL(COMM,0))*12 DESC
    -- ex.COMM이 NULL이거나 0인 사원중 ,
        -- SAL이 1200이상인 사원의 이름, 급여, 부서번호, 부서명,상여
        -- 부서명순, 급여 큰순 정렬
        SELECT ENAME, SAL, D.DEPTNO, DNAME, COMM
            FROM EMP E, DEPT D
    ...
    ­- 4. Comm이 null이 아닌 사원의 이름, 급여, 부서코드, 근무지를 출력하시오.
...
--탄탄3. 이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력. 
    ...
    SELECT ENAME, SAL, GRADE, (sal+comm)*12 연봉, DNAME
    FROM EMP E, DEPT D, SALGRADE 
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME 
    ...
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO;
    --ex. 'SMITH'의 상사는 FORD다' 포맷으로 출력
    SELECT W.EMPNO || '의 상사는 ' || M.ENAME || '다' MESSAGE
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO;
...
-- ▶ 4. OUTER-JOIN : EQUI/SELF-JOIN시 조건에 만족하지 않는 행까지 나타나게(데이터가 부족한쪽도 나오게)
...
SELECT * FROM EMP;  
    ...
    SELECT W.ENAME, W.SAL, W.JOB, NVL(M.ENAME,'없다') MANAGER
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO(+);           
    ...
    SELECT W.ENAME, W.SAL, GRADE, D.DNAME,(W.SAL+NVL(W.COMM,0))*12 연봉, M.ENAME
        FROM EMP W, EMP M, DEPT D, SALGRADE
        WHERE W.DEPTNO = D.DEPTNO
          AND W.SAL BETWEEN LOSAL AND HISAL
          AND W.MGR = M.EMPNO(+);
    ...
    SELECT W.ENAME 사원, M.ENAME MANAGER, MM.ENAME TOPMANAGER
