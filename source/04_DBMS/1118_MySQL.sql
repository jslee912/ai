...
-- DDL(제약조건 FK, 시퀀스없음)
-- DML(and:&&, or:||, 연결연산자대신 cncat사용, outer join, 단일행함수)
-- -----------
-- ※ DCL ※ --
-- -----------
create user user01 identified by 'password';
grant all privileges on *.* to user01;  -- 권한부여 (모든 스키마의 모든 객체에 대해)
...
show tables;  -- 현재 데이터베이스 내의 테이블들 // select * tab;
create table emp(
  empno numeric(4) primary key, -- 숫자4자리
  ename varchar(6) not null,    -- 바이트수가 아니고 글자수
...
insert into student values (103,'김길동',2);
insert into student values (104,'박길동',3);
select * from student;
...
create table student(
  ...
  mcode int ,
  primary key(sno),
  foreign key(mcode) references major(mcode)  -- FK조건은 아래 기술
);
insert into major(mname, moffice) values ('컴공','901호');
insert into major(mname, moffice) values ('인공지능','a601호');
insert into major(mname, moffice) values ('빅데이터','b601호');
select * from major;
insert into student values (101,'홍길동',1);
...
-- 워크벤치에서 자동 commit mode 운용(default)
...
select p.pno 사번,  p.pname 이름,
       pp.pname 상사이름,  d.dname 부서명
	from personal p	left join personal pp
	on p.manager = pp.pno   join division d
	on p.dno = d.dno  
	order by p.pno;  
-- (1)
select w.pno 사번, w.pname 이름, ifnull(m.pname,'★CEO★') 상사이름, dname
...
select pname 이름  from personal  where upper(pname) like 'S%';
select pname 이름  from personal  where substr(upper(pname),1,1) = 'S';  
select pname 이름  from personal  where instr(upper(pname),'S') = 1;    
-- 10. 사번, 이름, 급여, 부서명, 상사이름
select w.pno,  w.pname, d.dname, m.pname 상사이름
	from personal w
	join personal m
		on w.manager = m.pno   
	join division d
